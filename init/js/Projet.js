function init() {
    ...
    //Objets
    piste = new Piste(paramPiste); //Création de la piste
    ...
    function addAndRemovePiste() {
        if (!etat_partie) { //Si la partie est en cours, on ne replace pas le balai au point de départ
            ...
            piste = piste = new Piste(paramPiste); //On créé une nouvelle piste avec les paramètres du GUI
            scene.add(piste); //On ajoute la nouvelle piste à la scène
            placment_balai();
        } else {
            if (alertmess) {
                alert("Impossible partie commencé");
